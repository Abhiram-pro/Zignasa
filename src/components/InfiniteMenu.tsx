...
void main() {
    ...
    // Add thin purple glowing ring
    vec2 center = vec2(0.5, 0.5);
    float dist = distance(vUvs, center);
    float ringRadius = 0.47;
    float ringWidth = 0.008;
    
    // Create thin ring with glow
    float ringDist = abs(dist - ringRadius);
    float ring = smoothstep(ringWidth, 0.0, ringDist);
    float glow = smoothstep(ringWidth * 3.0, 0.0, ringDist) * 0.5;
    
    // Purple color with glow
    vec3 purpleColor = vec3(0.45, 0.13, 0.7); // #7321B3 purple
    vec3 glowColor = vec3(0.6, 0.3, 0.9); // Lighter purple for glow
    
    // Combine ring and glow
    float totalRing = ring + glow;
    outColor.rgb = mix(outColor.rgb, mix(glowColor, purpleColor, ring / max(totalRing, 0.001)), totalRing);
    
    // Smooth edge fade
    float edgeFade = smoothstep(0.51, 0.49, dist);
